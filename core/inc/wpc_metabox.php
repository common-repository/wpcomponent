...
class wpcomponent_metabox
{
    ...
    public function wpcomponent_addMetaBox_Sidebar_callback(){
        ...
        $wpc_folder = get_stylesheet_directory() . '/' . WPCOMPONENT_FOLDER;
        $wpc_folder_uri = get_stylesheet_directory_uri() . '/' . WPCOMPONENT_FOLDER;
            ...
            <?php
                ...
                foreach ($plugin_template as $key => $value) {
                    ...
                    if( gettype($value) === "array" ){
                        ...
                        if( get_option( 'wpcomponent_setting_disable_plugin_component' ) != 'true' ){
        ...
        <ol id="wpcomponent-theme-<?=$key_parent?>"<?=$class?>>
                    ...
                    ?>
                            ...
                            <li>
                                ...
                                data-slugs="<?=$slugs?>"
                                <?php 
                                    if(file_exists($wpc_folder.'/'.$key_parent.'/'.preg_replace('/\\.[^.\\s]{3,4}$/', '',$template->file).'.png')):
                                ?>
                                data-pic="<?=$wpc_folder_uri.'/'.$key_parent.'/'.preg_replace('/\\.[^.\\s]{3,4}$/', '',$template->file)?>.png"
                                <?php endif; ?>
                                title="<?=$template->description?>" class="wpc_admin--item">
                                    <h4><?=$template->name?></h4>
                                </a>
                            </li>
